...
export const HomePage = () => {
  ...
  const [priceRange, setPriceRange] = useState(1000000); // Max precio inicial
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        // Excluimos los "Servicios" del catálogo de ventas directo si quieres
        const salesProducts = data.filter((p: Product) => p.category !== 'Servicios');
  ...
  return (
    <Container className="py-5">
      ...
      <Row>
        {/* COLUMNA FILTROS LATERAL (Solo visible en desktop grande o colapsable) */}
        ...
        <Col lg={9}>
                  ...
                  <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.3 }}>
                    <Card className="h-100 border-0 shadow-sm overflow-hidden hover-card">
                      <div className="position-relative" style={{ height: 220 }}>
                        <Card.Img variant="top" src={product.images[0]} className="h-100 object-fit-cover" />
                        {product.is_on_offer && <Badge bg="danger" className="position-absolute top-0 end-0 m-2">OFERTA</Badge>}
                      </div>
                      <Card.Body className="d-flex flex-column">
                        ...
                        <div className="mt-auto pt-3">
                          <h4 className="fw-bold text-primary mb-3">${Number(product.price).toLocaleString()}</h4>
                          <div className="d-grid gap-2">
                             <Button variant="primary" onClick={() => addToCart(product)}>Añadir</Button>
                             <Link to={`/product/${product.id}`} className="btn btn-outline-secondary btn-sm">Ver Detalle</Link>
                          </div>
                        </div>
                      </Card.Body>
                    </Card>
           ...
           {filteredProducts.length === 0 && (
              <div className="text-center py-5">
                 ...
                 <Button variant="link" onClick={() => {setCategoryFilter('Todas'); setSearchTerm('');}}>Ver todos</Button>
              </div>
           )}
        </Col>
      </Row>
